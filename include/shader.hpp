...
Shader::Shader(const char* vertexPath, const char* fragmentPath) {
	...
	try
	{
		...
		std::cout << "shader read error just dropped schlawg fix yo shit" << std::endl;
	}
	...
	if (!success) {
		...
		std::cout << "Vertex shader compile is fucked fr fr ong" << infolog << std::endl;
	}
	...
	if (!success) {
		...
		std::cout << "Fragment shader is gablonkywonked, try again later(compile issue with fragment shader)" << infolog << std::endl;
	}
	...
	if (!success) {
		...
		std::cout << "something wrong wtih shader program idfk what tho, attachment issues mayhaps? figure it out yourself idiot take the log" << infolog << std::endl;
	}
